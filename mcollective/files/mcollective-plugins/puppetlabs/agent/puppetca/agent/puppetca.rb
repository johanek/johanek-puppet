module MCollective
  module Agent
    class Puppetca<RPC::Agent
      ...
      def startup_hook
        @puppetca = @config.pluginconf["puppetca.puppetca"] || "/usr/bin/puppet"
      ...
      action "revoke" do
        ...
        reply[:out] = run("#{@puppetca} cert --color=none revoke '#{request[:certname]}'", :stdout => :output, :chomp => true)
      end
      ...
      action "sign" do
        ...
        if cert_waiting?(certname)
          reply[:out] = run("#{@puppetca} cert --color=none sign '#{request[:certname]}'", :stdout => :output, :chomp => true)
        else
